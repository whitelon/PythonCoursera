...
class ServerSocket:
    ...
    def send(self, data):
        if data in self.rsp_map:
            ...
            raise ServerSocketException(
                f"запрос не соответствует протоколу: {data}")
    ...
    def recv(self, bytes_count):
        try:
            ...
            raise ServerSocketException(
                "нет данных в сокете для чтения ответа")
...
class TestClient(unittest.TestCase):
    ...
    def test_client_get_client_error(self):
        try:
            ...
            message = exp.args[0]
            self.fail(
             f"Некорректно обработано сообщение сервера об ошибке: {message}")
